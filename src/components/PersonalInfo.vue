    ...
    <div class="tbl-person">
      <b-table bordered :items="items" :fields="fields">
        <template slot="change" slot-scope="item">
          <router-link :to ='{name: "editinfo", params: {id: 5}}'><b-button size="sm" class="mr-1" @click="editInfo(item)">Edit</b-button></router-link>
          <b-button size="sm" class="mr-1" @click="deleteInfo(item)">Delete</b-button>
        </template>
      </b-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    editInfo: function(item) {
      this.edit_id = item.item.no;
    },
    deleteInfo: function(item) {
      let selIndex = this.allItems.indexOf(item.item);
      this.allItems.splice(selIndex, 1);
      this.totalRows = this.allItems.length;
      let from_page = this.perPage * (this.pageNum - 1);
      let to_page = this.perPage * this.pageNum;
      this.items = this.allItems.filter(function(item) {
